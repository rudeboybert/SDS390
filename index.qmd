...
1. Recorded lecture posted in Screencasts tab of webpage (top right):
    1. FPP 8.2 Exponential smoothing with trend
    1. FPP 8.3 Exponential smoothing with seasonality
1. PS4 to be posted by the end of today
...
fit <- aus_economy |>
  model(
    ...
    AAN = ETS(Pop ~ error("A") + trend("Ad", phi = 0.7) + season("N"))
...
fit <- aus_holidays |>
  model(
    ...
    # additive = ETS(Trips ~ error("A") + trend("Ad") + season("A")),
    # multiplicative = ETS(Trips ~ error("M") + trend("Ad") + season("M"))
  )
