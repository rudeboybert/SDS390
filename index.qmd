...
```{=html}
<!--
1. Sometimes it doesn't make sense to study TS original values, but instead differences or % changes or returns of values. For example stock returns.
1. If for a lag value of $k$ the autocorrelation function is statistically indistinguishable from 0, then there is no information for forecasting.
